 ...
 */
  ...
  componentDidMount() {
    ...
    dispatch(queryBorrowList({ phone: Taro.getStorageSync('phone') }))
  }
  ...
  handleSwitchChange(v) {
    request('https://shiyunidt.cn/giveBackBook', { book_isbn: v.book_isbn }).then((res: any) => {
    ...
    return (
      <View>
        ...
        <View className='at-row at-row__justify--center' style={{ lineHeight: '100rpx', height: '100rpx' }}>
          ...
          <View className='at-col at-col-5' style={{ textAlign: 'center' }}>还书</View>
        </View>
        <AtList className="have-borrow-book">
          {borrowList.map((item, index) => (

            <View className="every-box">
              ...
              <AtButton
                ...
                type='secondary'
                size="small"
                onClick={() => this.handleSwitchChange(item)}
              >
                还书
              </AtButton>
            </View>
